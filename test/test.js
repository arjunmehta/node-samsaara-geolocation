...
test('Wait for X Connections', function(t) {
    ...
    fences['Connection Initialized'] = new TapeFence(1, function() {
        ...
        var connection = samsaara.connection('connection0');
        t.equal(typeof connection, 'object');
        t.equal(typeof connection.state, 'object');
        t.equal(typeof connection.state.position, 'object');
        t.equal(typeof connection.state.position.coords, 'object');
        t.equal(typeof connection.state.position.coords.latitude, 'number');
        t.equal(typeof connection.state.position.coords.longitude, 'number');
        t.equal(typeof connection.state.position.timestamp, 'number');

        debug('Client offset is:', samsaara.connection('connection0').state.position);
...
test('Update Position', function(t) {
    ...
    samsaara.connection('connection0').getCurrentPosition(function(err, position) {

        if (!err) {
            t.equal(typeof position, 'object');
            t.equal(typeof position.coords, 'object');
            t.equal(typeof position.coords.latitude, 'number');
            t.equal(typeof position.coords.longitude, 'number');
            t.equal(typeof position.timestamp, 'number');
            t.end();
        }
    });
});
